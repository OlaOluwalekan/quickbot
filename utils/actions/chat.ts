...
export const searchChat = async (
  ...
  let responses: ResponseProps[] = []

  // console.log('option', text, option, userId, chatId)
  // console.log('chatId=>', chatId)

  // filter all responses in the current chat with the search text in the response or question
  if (option === 'chat') {
  ...
  // filter all chats globally with the search text in the title
  const chats = await db.chat.findMany({
    where: {
      createdBy: userId,
      title: {
  ...
  // filter all responses globally with the search text in the response or question
  responses = await db.response.findMany({
    where: {
      chat: {
        createdBy: userId,
  ...
  const results: (ResponseProps | ChatProps)[] = [...chats, ...responses]
